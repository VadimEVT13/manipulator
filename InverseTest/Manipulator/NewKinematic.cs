...
namespace InverseTest.Manipulator
{
    public class NewKinematic
    {
        ...
        public NewKinematic(double X = 0, double Y = 0, double Z = 0)
        {
            base_point[0] = X;
            base_point[1] = Y;
            base_point[2] = Z;
        ...
        */
        ...
        Stack<double[]> newgP3(double[] P4, out double[] P34, double a, double b)
        {
            ...
            for (double i = -180 / 180.0 * Math.PI; i < 180 / 180.0 * Math.PI; i += 1 / 180.0 * Math.PI)
            {
        ...
        public Stack<double[]> newInverse(double X, double Y, double Z, double alf = 0, double bet = 0)
        {
            // Начальные данные
            ...
            foreach (double[] point in P3mass)                      // Для каждой такой точки P3 ищем решение кинематики
            {
                ...
                if (Math.Sqrt(point[0] * point[0] + point[1] * point[1] + (point[2] - l1) * (point[2] - l1)) <= l2 + l3)
                {
                    ...
                    if (P2 != null)
                    {
                        ...
                        o5 = o5 - Math.PI / 2;                      // Так как угол считался к P34 то нужно вычесть 90 градусов
                        ...
                        if (Math.Abs(R[0][0] - T[0][0]) < 0.01 & Math.Abs(R[1][0] - T[1][0]) < 0.01 & Math.Abs(R[2][0] - T[2][0]) < 0.01)
